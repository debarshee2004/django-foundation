...
class Subscription(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.stripe_id and self.pk:
            stripe_id = helpers.billing.create_product(
                name=self.name,
                metadata={"subscription_plan_id": str(self.pk)},
                raw=False,
            )
            self.stripe_id = stripe_id
            # Save again with the stripe_id
            super().save(*args, **kwargs)
...
class SubscriptionPrice(models.Model):
    ...
    def get_checkout_url(self):
        if self.pk:
            return reverse("sub-price-checkout", kwargs={"price_id": self.pk})
        return "#"
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.stripe_id and self.product_stripe_id is not None and self.pk:
            # Ensure interval is valid for Stripe API
            valid_intervals = ["day", "week", "month", "year"]
            interval_value = (
                self.interval if self.interval in valid_intervals else "month"
            )

            stripe_id = helpers.billing.create_price(
                currency=self.stripe_currency,
                unit_amount=self.stripe_price,
                interval=interval_value,  # type: ignore
                product=str(self.product_stripe_id) if self.product_stripe_id else None,
                metadata={"subscription_plan_price_id": str(self.pk)},
                raw=False,
            )
            self.stripe_id = stripe_id
            # Save again with the stripe_id
        ...
        if self.featured and self.subscription and self.pk:
...
class UserSubscriptionManager(models.Manager):
    ...
    def by_range(self, days_start=7, days_end=120, verbose=True):
        return self.get_queryset().by_range(
            days_start=days_start, days_end=days_end, verbose=verbose
        )

    def by_days_left(self, days_left=7):
        return self.get_queryset().by_days_left(days_left=days_left)

    def by_days_ago(self, days_ago=3):
        return self.get_queryset().by_days_ago(days_ago=days_ago)
