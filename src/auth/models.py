...
class CustomUser(AbstractUser):
    ...
    receive_marketing_emails = models.BooleanField(
        default=True, help_text="Whether user wants to receive marketing emails"
    )
    ...
    created_at = models.DateTimeField(
        default=timezone.now, help_text="When the account was created"
    )
    updated_at = models.DateTimeField(
        auto_now=True, help_text="When the account was last updated"
    )
...
class UserProfile(models.Model):
    ...
    bio = models.TextField(
        max_length=500, blank=True, help_text="User's biography or description"
    )
    ...
    # Timestamps
    created_at = models.DateTimeField(
        default=timezone.now, help_text="When the profile was created"
    )
    updated_at = models.DateTimeField(
...
class SocialAccount(models.Model):
    ...
    provider = models.CharField(
        max_length=20, choices=PROVIDER_CHOICES, help_text="Social media provider"
    )

    provider_id = models.CharField(
        max_length=100, help_text="User ID from the social provider"
    )

    provider_email = models.EmailField(
        blank=True, help_text="Email from the social provider"
    )

    provider_name = models.CharField(
        max_length=200, blank=True, help_text="Display name from the social provider"
    )

    avatar_url = models.URLField(
        blank=True, help_text="Avatar URL from the social provider"
    )
    ...
    extra_data = models.JSONField(
        default=dict, blank=True, help_text="Additional data from the social provider"
    )

    # Timestamps
    connected_at = models.DateTimeField(
        default=timezone.now, help_text="When this social account was first connected"
    )
...
class LoginAttempt(models.Model):
    ...
    # Attempt details
    email_attempted = models.EmailField(help_text="Email address used in login attempt")
    ...
    country = models.CharField(
        max_length=100, blank=True, help_text="Country of the IP address"
    )

    city = models.CharField(
        max_length=100, blank=True, help_text="City of the IP address"
    )

    # Timestamp
    attempted_at = models.DateTimeField(
        default=timezone.now, help_text="When the login attempt occurred"
    )
